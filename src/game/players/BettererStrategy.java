...
public class BettererStrategy implements Strategy {
	...
	public Integer[] determineMove(Board board, Stone color) {
		...
		if (!goodMoves.isEmpty()) {
			...
			Board deepCopy3 = board.deepCopy();
			int[] scores = board.determineScores();
			int currentScore;
			...
			for (Integer[] m : neutralMoves) {
				deepCopy3.addStone(m[0], m[1], color);
				int[] newScores = deepCopy3.determineScores();
				int newScore;
				if (color.equals(Stone.BLACK)) {
					newScore = newScores[0] - newScores[1];
				} else {
